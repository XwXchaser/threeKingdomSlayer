...
| 2 | [Core](Core.summary.md) | 9 | 阵型数据结构、敌人/武将/关卡 ScriptableObject 配置、排位置计算、关卡配置管理器、存档系统 |
...
| 7 | [UI](UI.summary.md) | 5 | 战斗 HUD、场景转场模糊动画、浮动伤害跳字、主菜单（选关+存档）、精灵动画 |
...
`EnemyConfig`, `HeroConfig`, `StageConfig`(+`WaveConfig`/`RowConfig`), `RowFormationPreset`, `StageRegistry`

**静态工具**（无 MonoBehaviour 依赖）：`RowFormation`, `SaveManager`

**场景级管理器**（MonoBehaviour singleton）：`StageConfigManager` (MainMenu)

**UI 层**（读取 singleton，写入屏幕）：`BattleHUD`, `CameraManager`, `MainMenuUI`, `PingPongAnim`
...
- **存档**：`PlayerPrefs` + `JsonUtility` 序列化

## 核心设计模式

- **Singleton**：`ColumnManager`, `EnemyPool`, `DamageNumberManager`, `EnemyManager`, `StageController`, `AttackSystem`, `InputManager`, `PlayerState`, `WaveSpawner`, `StageConfigManager`
