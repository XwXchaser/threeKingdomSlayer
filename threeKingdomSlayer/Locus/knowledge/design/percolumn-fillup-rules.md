...
- `RowConfig.IsExplicitEmptyGate` 严格识别 `(0,0,0,0,0)`。
- `WaveSpawner` 在 PerRow 波次生成前注册本波排布局与敌人原始配置排；PerColumn 清理布局。
- `ColumnManager` 保存显式空排、阻挡排和已开启门状态；前方配置排仍存活时，后方敌群不能越过节奏门。
- 门开启后，连续空排由同一 WaveMarch 计划连续通过，不再在每个空排之间使用普通 `rushMoveDelay`。
- 连续计划使用独立 `RushMoveMode.ContinuousEmptyRun`，不改变 PushReturn、Boss 和旧 PerColumn 调度语义。
- WaveMarch 规划阶段预计算目标排和目标槽，活动阶段保留 owner/generation 与目标槽集合；敌人死亡或位移移除活动成员后重新规划。
- 已执行 Unity 完整重编译；输入系统和 `AttackSystem` 未修改。当前已完成代码级验证，仍需在 Stage_Trial、带配置空排测试波次及击退/击飞并发场景进行 Play Mode 回归。
...
- 当前实现：配置布局模式下先规划全局空排、已开放节奏门、每个敌人的目标排和目标槽，再创建同一 generation 的连续 WaveMarch 订单；连续通过空排时不使用普通 `rushMoveDelay`。
- 约束：仍只在合法全局空排形成后推进；不退化为按列紧凑，不移动未受影响敌人。

### 已实现：配置空排节奏门

- `RowConfig.IsExplicitEmptyGate` 严格识别 `(0,0,0,0,0)`。
- `WaveSpawner` 保存本波配置排布局和敌人原始配置排，`ColumnManager` 依据前方配置排存活状态控制门的开启。
- 前方敌群未清空时，后方敌群不能越过配置空排；门开启后连续空排由同一补齐计划连续跑完。

### 待验收：新波次远处入场与战斗期补齐边界

- 当前仍需要在正式流程中确认新波次远处入场视觉与战斗期 WaveMarch 不互相影响。
- 不允许通过直接写 `rowIndex` 实现闪现入场；入场与战斗补齐必须继续保持独立语义。
...
已实现配置布局模式下的批量规划与连续移动：先依据全局空排和配置空排门计算目标排并预留目标槽，再创建同一 generation 的 WaveMarch 订单。实现未直接批量写 `rowIndex`，未修改输入链路；运行时并发边界仍按上方待验收项回归。
