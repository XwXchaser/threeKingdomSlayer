...
- Pierce visual shutdown is range-end synchronized without changing combat authority: when `AttackWave` reaches the formation-derived Pierce visual end position, `PierceAxialSpinVisual.BeginEndFade(0.35f)` fades the crossed-plane weapon, vortex and blur; `PierceVortexVisual.StopAfterimages()` stops new afterimages. Existing `PierceVortexAfterimage` instances retain an owner reference and are force-faded in 0.12s only for that Pierce, preventing long forward-flying trails and avoiding interference between simultaneous Pierce attacks.
