...
| `StageController` (MonoBehaviour, singleton) | 顶层关卡流程。Start 时自动开始关卡。管理 `StageState` 转换（None -> InProgress -> Victory/Defeat）。连线事件：`WaveSpawner.OnAllWavesCompleted` -> Victory；`PlayerState.OnPlayerDied` -> Defeat；`EnemyManager.OnAnyEnemyDied` -> 加击杀/金币。暴露阵型参数。处理 MainMenu 和 Battle 场景加载。 |
...
- **StageController**：`StageConfig` (Core), `RowFormation` (Core), `WaveSpawner` (Wave), `EnemyManager`, `PlayerState`, `EnemyPool`, `Enemy` + Unity `SceneManager`
