...
| `InputManager` (MonoBehaviour, singleton) | 统一鼠标 + 触摸输入。手势识别：点击 -> Stab，长按（>=0.3s）-> Pierce（若充能 >=0.5s），带角度分析的滑动 -> Launch（近垂直）、Sweep（近水平）、Slash（斜向/默认）。无充能快速滑动 -> Slash。暴露 `OnChargeBegan/Updated/Ended` 供 UI 充能指示器使用。列映射：屏幕坐标投影到世界坐标，匹配最近存活敌人所在列。 |
