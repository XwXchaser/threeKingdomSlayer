...
| 类 | 说明 |
|---|---|
| `WaveSpawner` (MonoBehaviour, singleton) | 从 `StageConfig` 波次生成敌人。`SpawnNextWave()` 遍历波次排，调用 `SpawnRow()` 进行：计算总槽位数以居中、决定共享 rowIndex（列内最大 + 可选偏移 + 非 boss 波 +2）、从对象池取敌人、初始化、向 EnemyManager 注册。生成后对所有列触发 `TriggerFillForward()`。协程 `WaitForWaveClearAndNotify()` 每 0.5s 轮询 `EnemyManager.IsAllEnemiesDead` 后触发 `OnWaveCompleted`。事件：`OnWaveStarted`, `OnWaveCompleted`, `OnAllWavesCompleted`。 |
...
- `EnemyConfig`（Core）— 缓存在 `enemyConfigCache` 静态字典中。优先 Inspector 拖拽的 `enemyConfigs` 列表，回退到 `Resources.LoadAll<EnemyConfig>()`
- `EnemyPool`（Enemy）— `GetEnemy()`, `ReturnEnemy()`
...
- **静态配置缓存在 Start 时清除**：每次场景加载时 `enemyConfigCache.Clear()`，避免场景重载后配置过期
