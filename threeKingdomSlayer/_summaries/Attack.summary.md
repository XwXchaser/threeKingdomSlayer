...
- Fixed 波：按 `rowIndex` 以伤害类型特定延迟错开命中。代码声明了 5 个错开常量：`StabStagger=0.03f`、`SlashStagger=0.05f`、`PierceStagger=0.02f`、`SweepStagger=0.06f`、`LaunchStagger=0.04f`。实际 Fixed 模式仅使用 Stab/Slash/Launch（Pierce/Sweep 为 Travel 模式用不到）
- 波颜色按类型由 `private static GetColor(DamageType)` 编码（Stab=黄, Slash=蓝, Pierce=绿, Sweep=红, Launch=紫）
- 无存活目标时波在 0.2s 内自毁
