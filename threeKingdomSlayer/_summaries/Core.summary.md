...
| `AttackSkillConfig` (ScriptableObject) | 攻击技能配置：`attackType`（枚举）、`damageType`、`damage`、`poiseDamage`、`rangeRows`、`cooldown`、`launchDuration`、`attackWavePrefab`、`isUltimate`、`ultimateEnergyCost`、`ultimateEnergyGain`。每个技能一个 .asset，策划可拖拽装配。菜单：`Assets > Create > 一夫当关/攻击技能配置` |
| `HeroConfig` (ScriptableObject) | 武将数值：`heroId`、血量、复活次数、复活血量百分比。`List<AttackSkillConfig> skillConfigs` 技能装配列表 + `GetSkillConfig(AttackType)` 查询方法。全局 `damageBonusPercent`。菜单：`Assets > Create > 一夫当关/武将配置` |
...
- `UltimateSystem` 为 MonoBehaviour singleton，依赖 `UltimateEffect` 抽象类、`PlayerState`（读取 heroConfig.skillConfigs 获取 ultimateEnergyGain）
...
- **新武将**：创建 `HeroConfig` 资产，创建所需 `AttackSkillConfig` 资产拖入 `skillConfigs` 列表，赋给 `PlayerState.heroConfig`
